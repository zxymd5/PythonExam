...
class Profile(CreateUpdateMixin):
    ...
    uid = ShortUUIDField(_(u'用户id'), max_length=32, blank=True, null=True, help_text=u'用户唯一标识', db_index=True,
                         unique=True)
    user_src = models.IntegerField(_(u'用户来源'), choices=USER_SRC, default=GUEST_USER, help_text=u'用户来源',
                                   db_index=True)
    user_status = models.IntegerField(_(u'用户状态'), choices=USER_STATUS, default=ACTIVATED)

    unionid = models.CharField(_(u'微信unionid'), max_length=32, blank=True, null=True, help_text=u'微信Unionid',
                               db_index=True)
    openid = models.CharField(_(u'微信openid'), max_length=32, blank=True, null=True, help_text=u'微信授权Openid',
                              db_index=True)
    ...
    def upgrade_data(self):
        return {
            ...
            'upgrade_time': tc.datetime_to_string(
                tc.to_local_datetime(self.upgrade_time)) if self.final_is_upgrade and self.upgrade_time else '',
            'expire_time': tc.datetime_to_string(
                tc.to_local_datetime(self.expire_time)) if self.final_is_upgrade and self.expire_time else '',
...
class UserInfo(CreateUpdateMixin):
    ...
    graduated_from = models.CharField(_(u'毕业院校'), max_length=60, blank=True, null=True,
                                      help_text=u'graduated_from/毕业院校')
